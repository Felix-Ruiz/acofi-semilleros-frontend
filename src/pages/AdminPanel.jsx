...
function AdminPanel() {
  ...
  const enviarCorreos = async (id_ponencia = null) => {
    ...
    if (!confirmacion) return;
    setProcesandoAccion(true);
    setMensaje({ tipo: '', texto: 'Enviando correos a los integrantes, esto puede tardar un momento...' });
    try {
    ...
    <div className="max-w-7xl mx-auto bg-white p-4 md:p-10 rounded-2xl shadow-xl border border-gray-100 w-full overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-4 mb-6 border-b pb-4">
        ...
        {vistaActual === 'ponencias' && (
          <div className="flex flex-col md:flex-row gap-2 w-full lg:w-auto">
            <input 
              type="text" 
              placeholder="Buscar título, integrante, código..." 
        ...
        {vistaActual === 'evaluadores' && (
          <div className="flex flex-col md:flex-row gap-2 w-full lg:w-auto">
            <input 
              type="text" 
              placeholder="Buscar evaluador, documento, correo..." 
      ...
      {cargando ? (
        <div className="text-center py-12">
          <p className="text-blue-900 font-bold text-lg mb-2 animate-pulse">Cargando datos del servidor...</p>
        </div>
      ) : (
                  ...
                  {ponenciasMostradas.map((p) => (
                    <tr key={p.id} className="border-b border-gray-200 hover:bg-gray-50 transition-colors text-sm">
                      ...
                      <td className="p-4">
                        ...
                        <p className="text-xs text-gray-500 mt-1">{p.estudiante_institucion}</p>
                      </td>
              ...
              <table className="w-full text-left border-collapse min-w-200">
                <thead>
                  <tr className="bg-blue-950 text-white text-sm">
                    ...
                    <th className="p-4 font-semibold w-1/3 rounded-tr-xl">Proyecto al que Pertenece</th>
                  </tr>
                </thead>
                <tbody>
      ...
      {modalConfirmacion.abierto && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 border-t-4 border-red-600 animate-fadeIn text-center">
            ...
            <p className="text-gray-500 text-sm mb-6">Esta acción es irreversible.</p>
      ...
      {modalAbierto && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
            <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 border-t-4 border-blue-600 animate-fadeIn text-center">
                <p className="text-gray-600">Por favor, utilice la subida por Excel para los estudiantes o pídales que se registren desde la plataforma web.</p>
                <button onClick={() => setModalAbierto(false)} className="mt-4 px-5 py-2.5 bg-blue-900 text-white rounded-lg font-bold">Cerrar</button>
            </div>
        </div>
      )}
