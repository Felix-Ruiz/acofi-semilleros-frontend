...
function AdminPanel() {
  ...
  const [vistaActual, setVistaActual] = useState('ponencias'); // ponencias | evaluadores | ranking | qr
  ...
  // ESTADO NUEVO: Para el Modal de Confirmación de Eliminación
  ...
  const urlRegistroEvaluador = "https://subdominio.acofiapps.com/registro-evaluador";
  ...
  const cargarDatos = async () => {
    try {
      ...
      if (vistaActual === 'ponencias') {
        const res = await axios.get('https://acofi-backend.onrender.com/api/admin/ponencias');
        setPonencias(res.data);
      } else if (vistaActual === 'evaluadores') {
        const res = await axios.get('https://acofi-backend.onrender.com/api/admin/evaluadores');
        setEvaluadores(res.data);
      } else if (vistaActual === 'ranking') {
        const res = await axios.get('https://acofi-backend.onrender.com/api/admin/ranking');
        setRanking(res.data);
      }
    } catch (error) {
  ...
  const aprobarPonencia = async (id) => {
    try {
      const respuesta = await axios.post(`https://acofi-backend.onrender.com/api/admin/aceptar_ponencia/${id}`);
  ...
  // Función para abrir el Modal de Confirmación (Reemplaza a window.confirm)
  const solicitarEliminacion = (entidad, id) => {
    setModalConfirmacion({ abierto: true, entidad, id });
  };

  // Función que realmente ejecuta la eliminación en el backend
  const confirmarEliminacion = async () => {
    const { entidad, id } = modalConfirmacion;
    setModalConfirmacion({ abierto: false, entidad: '', id: null }); // Cerrar modal
    setMensaje({ tipo: '', texto: '' });
    
    try {
      await axios.delete(`https://acofi-backend.onrender.com/api/admin/${entidad}/${id}`);
  ...
  const guardarModal = async (e) => {
    e.preventDefault();
    const urlBase = `https://acofi-backend.onrender.com/api/admin/${modalEntidad === 'ponencia' ? 'ponencias' : 'evaluadores'}`;
  ...
  const descargarExcel = (entidad) => {
    window.open(`https://acofi-backend.onrender.com/api/admin/exportar/${entidad}`, '_blank');
  };

  return (
    <div className="max-w-7xl mx-auto bg-white p-6 md:p-10 rounded-2xl shadow-xl border border-gray-100">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 border-b pb-4 gap-4">
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          <button onClick={() => descargarExcel('estudiantes')} className="px-3 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 font-medium transition-colors">Excel Estudiantes</button>
          <button onClick={() => descargarExcel('evaluadores')} className="px-3 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 font-medium transition-colors">Excel Evaluadores</button>
          <button onClick={() => descargarExcel('ponencias')} className="px-3 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 font-medium transition-colors">Excel Ponencias</button>
          <button onClick={() => descargarExcel('evaluaciones')} className="px-3 py-2 bg-blue-700 text-white text-sm rounded-lg hover:bg-blue-800 font-bold transition-colors">Descargar Resultados</button>
        </div>
      </div>

      {mensaje.texto && (
        <div className={`p-4 mb-6 rounded-lg font-medium text-center ${mensaje.tipo === 'exito' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'}`}>{mensaje.texto}</div>
      )}

      {/* PESTAÑAS Y BOTÓN AÑADIR */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 border-b pb-4">
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setVistaActual('ponencias')} className={`px-5 py-2.5 rounded-lg font-semibold transition-colors ${vistaActual === 'ponencias' ? 'bg-blue-950 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>Gestión de Ponencias</button>
          <button onClick={() => setVistaActual('evaluadores')} className={`px-5 py-2.5 rounded-lg font-semibold transition-colors ${vistaActual === 'evaluadores' ? 'bg-blue-950 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>Listado Evaluadores</button>
          <button onClick={() => setVistaActual('ranking')} className={`px-5 py-2.5 rounded-lg font-semibold transition-colors ${vistaActual === 'ranking' ? 'bg-amber-500 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>Resultados y Ranking</button>
          <button onClick={() => setVistaActual('qr')} className={`px-5 py-2.5 rounded-lg font-semibold transition-colors ${vistaActual === 'qr' ? 'bg-blue-950 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>QR Evaluadores</button>
        </div>

        {vistaActual === 'ponencias' && (
          <button onClick={() => abrirCrearModal('ponencia')} className="px-4 py-2.5 bg-blue-900 text-white font-bold rounded-lg hover:bg-blue-800 shadow-md transition-colors">+ Añadir Ponencia</button>
        )}
        {vistaActual === 'evaluadores' && (
          <button onClick={() => abrirCrearModal('evaluador')} className="px-4 py-2.5 bg-blue-900 text-white font-bold rounded-lg hover:bg-blue-800 shadow-md transition-colors">+ Añadir Evaluador</button>
        )}
      </div>

      {/* TABLAS */}
      {cargando ? (
        <p className="text-center text-gray-500 py-10">Cargando información...</p>
      ) : (
        <div className="overflow-x-auto">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-blue-950 text-white">
                    <th className="p-4 font-semibold rounded-tl-xl">Trabajo</th>
                    <th className="p-4 font-semibold">Estudiante / Institución</th>
                    <th className="p-4 font-semibold">Estado</th>
                    <th className="p-4 font-semibold">Código / QR</th>
                ...
                <tbody>
                  {ponencias.map((p) => (
                    <tr key={p.id} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                      <td className="p-4"><p className="font-semibold text-gray-800 line-clamp-2">{p.titulo}</p></td>
                      <td className="p-4"><p className="text-gray-800 font-medium">{p.estudiante_nombre}</p><p className="text-sm text-gray-500">{p.estudiante_institucion}</p></td>
                      ...
                      <td className="p-4">
                        {p.codigo ? (
                          <div>
                            <span className="font-mono bg-gray-200 px-2 py-1 rounded text-gray-800 font-bold block mb-1 text-center w-max">{p.codigo}</span>
                            <a href={`https://acofi-backend.onrender.com${p.url_qr}`} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline">Ver QR</a>
                          </div>
                        ) : <span className="text-gray-400 italic text-sm">Sin asignar</span>}
                      </td>
                      <td className="p-4 text-center space-y-1">
                        {p.estado === 'pendiente' && (
                          <button onClick={() => aprobarPonencia(p.id)} className="block w-full px-2 py-1 bg-blue-600 text-white rounded text-xs font-medium transition-colors">Aprobar</button>
                        )}
                        <button onClick={() => abrirEditarModal('ponencia', p)} className="block w-full px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs font-medium transition-colors">Editar</button>
                        {/* Cambio: Ahora llama al Modal propio */}
                        <button onClick={() => solicitarEliminacion('ponencias', p.id)} className="block w-full px-2 py-1 bg-red-100 text-red-600 rounded text-xs font-medium transition-colors">Eliminar</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          {vistaActual === 'evaluadores' && (
            evaluadores.length === 0 ? (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-blue-950 text-white">
                    <th className="p-4 font-semibold rounded-tl-xl">Nombre del Evaluador</th>
                    <th className="p-4 font-semibold">Documento</th>
                    <th className="p-4 font-semibold">Institución / Cargo</th>
                    <th className="p-4 font-semibold text-center rounded-tr-xl">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {evaluadores.map((e) => (
                    <tr key={e.id} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                      <td className="p-4"><p className="font-semibold text-gray-800">{e.nombres_apellidos}</p><p className="text-sm text-gray-500">{e.correo}</p></td>
                      <td className="p-4 text-gray-700 font-mono">{e.documento_identidad}</td>
                      <td className="p-4"><p className="text-gray-800 font-medium">{e.institucion}</p><p className="text-sm text-gray-500">{e.cargo} | <span className="italic text-blue-900">{e.evento.split(',')[0]}</span></p></td>
                      <td className="p-4 text-center space-y-1">
                        <button onClick={() => abrirEditarModal('evaluador', e)} className="block w-full px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs font-medium transition-colors">Editar</button>
                        {/* Cambio: Ahora llama al Modal propio */}
                        <button onClick={() => solicitarEliminacion('evaluadores', e.id)} className="block w-full px-2 py-1 bg-red-100 text-red-600 rounded text-xs font-medium transition-colors">Eliminar</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {ranking.map((item, idx) => (
                    <tr key={item.id} className={`border-b border-gray-200 ${idx === 0 ? 'bg-amber-50' : ''}`}>
                      <td className="p-4 text-center font-bold text-gray-700">{idx === 0 ? '🥇 1' : idx === 1 ? '🥈 2' : idx === 2 ? '🥉 3' : idx + 1}</td>
                      <td className="p-4"><p className="font-semibold text-gray-800">{item.titulo}</p><p className="text-sm text-gray-500">Por: {item.estudiante_nombre} | Código: {item.codigo}</p></td>
                      <td className="p-4 text-center font-medium text-gray-700">{item.num_evaluaciones} / 2</td>
                      <td className="p-4 text-center"><span className="bg-amber-100 text-amber-800 px-3 py-1 rounded font-bold border border-amber-200">{item.promedio} pts</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {vistaActual === 'qr' && (
            <div className="flex flex-col items-center justify-center py-12 bg-gray-50 rounded-xl border border-gray-200">
              <h3 className="text-2xl font-bold text-blue-950 mb-4">Registro de Evaluadores</h3>
              <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-200 mb-6">
                <QRCodeSVG value={urlRegistroEvaluador} size={256} level="H" />
              </div>
              <p className="text-sm text-gray-500 font-mono bg-white px-4 py-2 rounded border border-gray-200">{urlRegistroEvaluador}</p>
            </div>
          )}
      ...
      {/* MODAL DE CONFIRMACIÓN DE ELIMINACIÓN (Reemplaza a window.confirm) */}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 border-t-4 border-red-600 animate-fadeIn text-center">
            ...
            <div className="flex justify-center gap-3">
              <button onClick={() => setModalConfirmacion({ abierto: false, entidad: '', id: null })} className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-semibold transition-colors">Cancelar</button>
              <button onClick={confirmarEliminacion} className="px-5 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 font-bold shadow-md transition-colors">Sí, eliminar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalAbierto && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 overflow-y-auto backdrop-blur-xs">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl p-6 md:p-8 border animate-fadeIn max-h-[90vh] overflow-y-auto">
            <h3 className="text-2xl font-bold text-blue-950 mb-2 capitalize">{modalModo} {modalEntidad === 'ponencia' ? 'Ponencia y Estudiante' : 'Evaluador'}</h3>
            ...
            <form onSubmit={guardarModal} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-4 border-t mt-6">
                <button type="button" onClick={() => setModalAbierto(false)} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-semibold text-sm transition-colors">Cancelar</button>
                <button type="submit" className="px-5 py-2 bg-blue-900 text-white rounded-lg hover:bg-blue-800 font-bold text-sm shadow-md transition-colors">Guardar Información</button>
              </div>
            </form>
          </div>
        </div>
